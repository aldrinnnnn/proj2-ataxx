...
class Board {
    ...
    Board(Board board0) {
        ...
        _numPieces = new int[] {board0.numPieces(EMPTY),
                board0.numPieces(BLOCKED),
                board0.numPieces(RED), board0.numPieces(BLUE)};
        _allMoves = new ArrayList<Move>();
        ...
        for (char c = 'a'; c <= 'g'; c++) {
            for (char r = '1'; r <= '7'; r++) {
                if (get(c, r) == who) {
                    for (int i = -2; i < 3; i++) {
                        for (int j = -2; j < 3; j++) {
                            if (get((char) (c + i), (char) (r + j)) == EMPTY) {
                                return true;
                            }
                        }
                    }
    ...
    void makeMove(Move move) {
        ...
        for (int i = -1; i < 2; i++) {
            for (int j = -1; j < 2; j++) {
                if (get((char) (move.col1() + i),
                        (char) (move.row1() + j)) == opponent) {
                    set((char) (move.col1() + i),
                            (char) (move.row1() + j), whoseMove());
                    _numPieces[_whoseMove.ordinal()] += 1;
        ...
        if ((numPieces(_whoseMove) == 0 || numPieces(opponent) == 0)
                || (_numJumps == JUMP_LIMIT) || !check()) {
            if (numPieces(RED) < numPieces(BLUE)) {
                _winner = BLUE;
            } else if (numPieces(RED) > numPieces(BLUE)) {
                _winner = RED;
    ...
    /** Returns whether or not, board is empty. */
    ...
    void undo() {
        ...
        Integer prevNumJumps = _undoNumJumps.pop();
        while (prevPiece != null && prevSquare != null) {
            unrecordedSet(prevSquare, prevPiece);
            prevPiece = _undoPieces.pop();
            prevSquare = _undoSquares.pop();
            _numJumps = prevNumJumps;
            prevNumJumps = _undoNumJumps.pop();
        }
        _whoseMove = _whoseMove.opposite();
    ...
    /** ADDED: Recounts number of RED and BLUE pieces
     * and sets it to numPieces. */
    void countColors() {
    ...
    boolean legalBlock(char c, char r) {
        ...
        int offsetX = 'd' - c;
        int offsetY = '4' - r;
        if (get(c, r) != EMPTY || get((char) ('d' + offsetX), r) != EMPTY
                || get((char) ('d' + offsetX), (char) ('4' + offsetY)) != EMPTY
                || get(c, (char) ('4' + offsetY)) != EMPTY) {
            return false;
        }
        return true;
    }
    ...
    void setBlock(char c, char r) {
        ...
        int offsetY = '4' - r;
        set(c, r, BLOCKED);
        set((char) ('d' + offsetX), r, BLOCKED);
        set((char) ('d' + offsetX), (char) ('4' + offsetY), BLOCKED);
        set(c, (char) ('4' + offsetY), BLOCKED);
